...
When the CVM is utilizing a vTPM, the Agent extends the PCR 16 register with the hash value of the computation manifest (you can read more on the vTPM PCRs in the [measured boot](/trusted-execution/attestation/attestation-measured-boot) section of attestation). By extending the PCR 16 register, the hash of the computation manifest is a part of the [attestation report](/trusted-execution/attestation/attestation-introduction). This allows the user to verify all computations executed in the CVM and ensures that the Agent they are communicating with will execute the specified computation.
